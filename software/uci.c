//Управляющая программа микроконтроллера ATmega8
//устройства цифровой индикации v.1.0
//Автор Захаров Т.Р. 2021 г.
...
unsigned char buffer[6]={99,12,13,14,15,99};    //буфер экрана
unsigned char dir[10]={2,2,2,2,2,2,2,2,2,2};    //буфер направлений
...
//массив данных знакогенератора символов цифр и знаков
__flash const char img[16][100]={{			
...
//массив данных знакогенератора символа запятой
__flash const char dotim[5]={0xC0,0xF8,0xFE,0x7E,0x1E};	

//массив данных знакогенератора надписей М1 - М9
__flash const char mem[9][25]={
...
//массив данных знакогенератора стрелки направления
__flash const char arrow[25]=				
...
void initialize(void)						//функция инициализации
{
  ...
  while(EECR&0x02);
  k=0;
  for(i=0;i<4;i++)							//читаем 4 байта числа шага
  {
    EEAR=i;
    EECR|=0x01;
    tmp=EEDR;
  ...
  EEAR=4;
  EECR|=0x01;
  if((EEDR!=k)||(step==0)||((PINC&0x10)==0))	//если произошла ошибка чтения,
  {															//прочитан 0 или нажата кнопка SEL
    beep=1;											//запускаем процедуру ввода нового
    buffer[0]=99;
    buffer[1]=99;
    buffer[2]=99;
    buffer[3]=99;
    ...
    m=0;
    k=0;
    for(i=0;i<4;i++)						//сохраняем новое число в памяти
    {
      while(EECR&0x02);
      EEAR=i;
...
#pragma vector=TIMER0_OVF_vect	//обработчик прерывания по 
__interrupt void tim0(void)			//переполнению таймера 0
{
  if(!(PINC&0x10))							//опрос кнопки SEL
    if(!m)											//запись точки предупреждения
    {
      data[0]=0;
      dir[0]=2;
    }
  ...
  enc=PIND&0x03;								//опрос ручки энкодера
            
  if((enc!=encp1)&&(enc!=encp2))//подавление дребезга энкодера
    enccnt++;
  else
    enccnt=0;
      
  if(enccnt>ENCDEL)							//если одно положение считано более
  {															//ENCDEL раз то расшифровка энкодера
    ...
    if(m==10)										//перемещение между экранами 
      m=0;											//М0-М9
    if(m==255)
      m=9;
    encp2=encp1;
...
#pragma vector=TIMER1_OVF_vect	//обработчик прерывания по
__interrupt void tim1(void)			//переполнению таймера 1
{
  if(beep)											//если установлен семафор beep
    PORTC^=0x20;								//выдаем звуковой сигнал и мигаем
  else
    PORTC|=0x20;
  return;
}

#pragma vector=INT0_vect				//обработчик внешнего прерывания
__interrupt void int0(void)
{
  unsigned char e=0;

DDRC=0x20;											//считываем счетчик энкодера
PORTD&=0xF7;
__delay_cycles(DELAY);
cnt0=PINC&0x0F;
PORTD|=0x08;
...
if(cnt0!=cnt1)									//вычисляем разницу и направление
{
  if(cnt0>cnt1)
  {
    e=cnt0-cnt1;
  ...
  cnt1=cnt0;
  if(e==8)											//если разница ровно 8  
    beep=1;											//импульсов выдаем предупреждение
  if(dir[0])
    data[0]+=e*step;
  else
    data[0]-=e*step;
...
unsigned char i,k,t,data;
  ...
  for(k=0;k<3;k++)							//цикл по микросхемам
  {
    PORTB=0x08<<k;							//выбор микросхемы
                          
    for(t=0;t<4;t++)						//цикл по строкам
    {
      PORTC&=0xF0;
      PORTD&=0x0F;
      PORTD|=(t<<6);
      ...
      PORTB|=0x04;
      for(i=0;i<25;i++)					//вывод первого знакоместа
      {
        if(buffer[k+k]<16)
        ...
        if((t==3)&&(i>19)&&(dot&(0x01<<(k+k))))
          data|=dotim[i-20];		//рисование десятичной точки
                          
        if((t==0)&&(k==0)&&m&&(!beep))
          data|=mem[m-1][i];		//рисование символа М...М9
              
        if((t==3)&&(k==0)&&(dir[m]<2))
          if(dir[m])						//рисование стрелки направления
            data|=arrow[i];
          else
            data|=arrow[24-i];
      ...
      for(i=0;i<25;i++)					//вывод второго знакоместа
      {
        if(buffer[k+k+1]<16)
        ...
        if((t==3)&&(i>19)&&(dot&(0x01<<(k+k+1))))
          data|=dotim[i-20];		//рисование днсятичной точки
